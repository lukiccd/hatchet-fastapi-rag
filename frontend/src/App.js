...
function App() {
  ...
  const fetchKnowledgeBases = async () => {
    try {
      const res = await fetch("http://localhost:3001/api/kb");
  ...
  const uploadDocument = async (e) => {
    ...
    alert(`Demo: uploaded "${uploadFile.name}" to ${uploadingKB}`);
    setUploadFile(null);
    setUploadingKB(null);
  };
  ...
  const sendMessage = async (e) => {
    ...
    // Simulate latency and fake AI reply
    setTimeout(() => {
      const botMessage = {
        sender: "bot",
        text:
